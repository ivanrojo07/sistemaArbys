    	...
    	if ($request->hasFile('sample_file')) {
    		...
    		if ($data->count()) {
    			...
    			foreach ($data as $key => $value) {
    				# code...
    				$arr[]=['clave'=>$value->clave,'descripcion'=>$value->descripcion, 'precio_lista'=>$value->precio_de_lista,'mensualidad_p_fisica'=>$value->pago_mensual_p_fisica,'mensualidad_p_moral'=>$value->p_moral,'apertura'=>$value->apertura,'inicial'=>$value->inicial,'marca'=>$value->marca, 'tipo'=>$value->tipo];
                }
                // dd($arr);
    			if (!empty($arr)) {
    				# code...
