					...
					<div id="seleccion" style="display: none;">
						<div class="row">
							...
							<div class="col-sm-3 form-group">
								...
								<input type="text" name="folio" id="folio" class="form-control" value="{{$cliente->vendedor->empleado->laborales->last()->oficina?$cliente->vendedor->empleado->laborales->last()->oficina->abreviatura:"OFIC1"}}{{$cliente->vendedor->id}}{{$cliente->transactions->last()->product->tipo[0]}}{{$folio + 1}}" required="" readonly="">
							</div>
...
<script type="text/javascript">
	...
	function destroy() {
		...
		@foreach($cliente->transactions as $transaccion)
			@if(count($transaccion->pagos) == 0 && isset($transaccion->product) && !empty($transaccion->product))
